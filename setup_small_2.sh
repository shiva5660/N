...
rm -rf nexus-network-intel-small
...
mv nex/nexus-network-intel-small .
mv nex/nexus-network-global .

# ---- Make binaries executable ----
chmod +x nexus-network-amd nexus-network-intel-small nexus-network-global

# ---- Cleanup ----
rm -rf nex

# ---- Ask for node number ----
read -p "Enter node number (line number from nodes_2.txt): " n
NODE_ID=$(sed -n "${n}p" nodes_2.txt)
if [ -z "$NODE_ID" ]; then
...
BOT1_TOKEN="8505428832:AAH-0wLgEr-kjnG4_DuXQwSZmGjvWI_DApI"
BOT2_TOKEN="8391643663:AAGK5Kh8mRFjTO1ZVf49oEZ5MzLlGDpvofE"
CHAT_ID="5765264116"
...
if [[ "\$CPU_VENDOR" == "AuthenticAMD" ]]; then
  ...
  CPU_BINARY="\$SCRIPT_DIR/nexus-network-intel-small"
else
